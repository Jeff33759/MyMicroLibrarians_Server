package jeff.authentication.aop;
...
public class ExceptionLoggingAspect {
	...
	/**
	 * 匹配jeff.authentication.service底下子目錄的所有類別的所有0~多個參數的方法。</p>
	 * */
	@Pointcut("execution(* jeff.authentication.service..*.*(..))")
	public void pointcutForService() {
	}
	
	/**
	 * 匹配jeff.authentication.common.util底下子目錄的所有類別的所有0~多個參數的方法。
	 * */
	@Pointcut("execution(* jeff.authentication.common.util..*.*(..))")
	public void pointcutForMyUtil() {
	}
	
	/**
	 * 匹配jeff.authentication.common.schedule底下子目錄的所有類別的所有0~多個參數的方法。
	 * */
	@Pointcut("execution(* jeff.authentication.common.schedule..*.*(..))")
	public void pointcutForSchedule() {
	}

	/**
	 * 匹配jeff.authentication.handler底下子目錄的所有類別的所有0~多個參數的方法。
	 * */
	@Pointcut("execution(* jeff.authentication.handler..*.*(..))")
	public void pointcutForHandler() {
	}
	 ...
	 * 因此需要log記錄下來。
	 * */
	@AfterThrowing(pointcut = "pointcutForService() || pointcutForMyUtil() || pointcutForHandler() || pointcutForSchedule()", 
			throwing = "throwable")
