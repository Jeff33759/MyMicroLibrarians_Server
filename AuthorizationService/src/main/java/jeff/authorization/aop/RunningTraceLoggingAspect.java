...
public class RunningTraceLoggingAspect {
	 ...
	 * 匹配列出的目錄，用於追蹤程式軌跡。</p>
	 * */
	@Pointcut("execution(* jeff.authorization.common.util..*.*(..))"
			+ "|| execution(* jeff.authorization.common.schedule..*.*(..))"
			+ "|| execution(* jeff.authorization.controller..*.*(..))"
			+ "|| execution(* jeff.authorization.service..*.*(..))"
			+ "|| execution(* jeff.authorization.handler..*.*(..))")
