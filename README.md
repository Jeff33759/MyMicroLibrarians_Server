...
市面上虛擬主機的發展已臻成熟，隨著雲端的使用愈趨普遍，加上前後端分離開發的趨勢，分散式的系統架構逐漸受到歡迎。

有別於傳統的單體式Web應用程式，分散式的系統架構除了可以幫助人們採用更加彈性的雲端部屬方案外，也有著更高的系統容錯性，缺點就是管理與開發要來得更加複雜。

本專案利用Spring Boot實作了分散式系統的模型，希望在學習使用Spring Cloud之前，能夠藉由實作本專案，更加了解分散式系統的運行機制。
...
4. _RefreshToken的密鑰及AccessToken的私鑰，也許不該經過傳輸的流程。_

目前的設計，是由Gateway啟動時順便生成各微服務所需的密鑰，並在各微服務向Gateway註冊成功時，Gateway再將密鑰序列化傳給所需的微服務使用，藉此實現「由Gateway統一生成並管理所有密鑰」的設計思維；可是說到底，非公鑰的密鑰可能不該經過這個傳輸的流程以免在傳輸過程中被竊取，所以也許可以更改系統架構，密鑰的產生就不由Gateway負責了，而是由各微服務啟動後自己產生就好，私鑰就由該微服務自己保管，公鑰則可以在註冊成功時順便傳給Gateway，若有其他微服務也需要那個公鑰，再由Gateway發放該公鑰...但這樣的設計，微服務與Gateway之間的交握註冊機制也許就要寫得更複雜一些，且要考慮到若微服務重新開機，密鑰是否會被刷新的情況，所以可能又要重新發放等等。

5. _基於Token的有狀態請求，可以改用redis實現Session共享_

一般使用JWT替代Session，是因為傳統基於Cookie的Session機制，並不能實現跨伺服端的Session共享；但現在有人使用redis實現跨伺服端的Session共享。

個人理解為把儲存Session的功能，再獨立拉出來一個伺服端做，一個Session可以對應多個SessionId(讓前端也可以多裝置共享Session)，然後該Session Server的資料庫使用速度較快的redis。前端請求時，將不再使用Cookie了，而是將Cookie中的SessionId儲存在localStorage中，每次發送請求都夾帶於標頭上(類似於Token做法)，假設伺服端實例A接收到SessionId，就會拿SessionId去跟Session Server要Session資料，當然伺服端實例B也能拿SessionId去跟Session Server要Session資料，兩個伺服端實例取出的都是同一個Session，也就實現了跨伺服端的Session共享。
...
JWT發放出去之後，不能透過ServerSide讓Token失效，必須等到exp時間過才會失去效用，所以黑名單機制無法立即作用於AccessToken。
