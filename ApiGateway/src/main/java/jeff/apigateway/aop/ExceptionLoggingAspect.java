...
public class ExceptionLoggingAspect {
	...
	private final Class<?>[] debugExTypes = new Class[]{
			...
			PathVarIsBlankException.class,
			RestTemplateErrorResException.class
	};
	...
	private final Class<?>[] warnExTypes = new Class[]{
			HasNotRegisteredException.class
	};
	...
	/**
	 * 匹配jeff.apigateway.service底下子目錄的所有類別的所有0~多個參數的方法。</p>
	 * */
	@Pointcut("execution(* jeff.apigateway.service..*.*(..))")
	public void pointcutForService() {
	}
	
	/**
	 * 匹配jeff.book.common.util底下子目錄的所有類別的所有0~多個參數的方法。
	 * */
	@Pointcut("execution(* jeff.apigateway.common.util..*.*(..))")
	public void pointcutForMyUtil() {
	}

	/**
	 * 匹配jeff.book.common.schedule底下子目錄的所有類別的所有0~多個參數的方法。
	 * */
	@Pointcut("execution(* jeff.apigateway.common.schedule..*.*(..))")
	public void pointcutForMySchedule() {
	}
	 ...
	 * 因此需要log記錄下來。
	 * */
	@AfterThrowing(pointcut = "pointcutForService() || pointcutForMyUtil() || pointcutForMySchedule()", 
			throwing = "throwable")
